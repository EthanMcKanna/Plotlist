...
export default function MutedShowsScreen() {
  ...
  // The row leaves on the tap; the mutation's rollback restores it on error.
  const handleUnmute = useCallback(
    async (showId: string) => {
      try {
        await unmuteShow.withOptimisticUpdate((localStore) => {
          const current = localStore.getQuery(api.notifications.getMutedShows, undefined);
          if (!Array.isArray(current)) return;
          localStore.setQuery(
            api.notifications.getMutedShows,
            undefined,
            current.filter((row: any) => row?.showId !== showId),
          );
        })({ showId });
  ...
  const renderItem = useCallback(
    ({ item }: { item: any }) => <MutedShowRow item={item} onUnmute={handleUnmute} />,
    [handleUnmute],
  );
